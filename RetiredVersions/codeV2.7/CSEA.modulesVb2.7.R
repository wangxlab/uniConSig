...
cal.uniConSig<-function(target.list,feature.list,preCalmatrix,minsize=20,weight.cut=0.05,power=1,root=0.5,ECNpenalty=1,method="Ochiai",rm.overfit=FALSE){ #method=="Ochiai" or "Jaccard"
...
cal.uniConSig.cut<-function(target.list,feature.list,preCalmatrix,minsize=10,rm.overfit=TRUE){
  uniConSig.all=cal.uniConSig(target.list=target.list,feature.list=feature.list,preCalmatrix=preCalmatrix,minsize=minsize,weight.cut=0.05,power=1,root=1,ECNpenalty=0.5,method="Ochiai",rm.overfit=rm.overfit) #method=="Ochiai" or "Jaccard"
...
pathwayAssociation<-function(topPathway,compare.list,feature.list,preCalmatrix,minsize=5,rm.overfit=FALSE){ #upPathways TRUE or FALSE
...
pathway.heatmap<-function(matrixData,clustering=TRUE,distanceMatric="euclidean",fontSize=5){
  ...
  #par(mar=c(2,2,2,2))
  if (clustering==TRUE){
    pathway.heatmap=pheatmap(matrixData,fontsize_row=fontSize,fontsize_col=fontSize,color=my_palette, cluster_rows=T, cluster_cols=T, clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean", clustering_method = "ward.D2") 
    # pathway.heatmap=heatmap.2(matrixData, distfun=function(matrixData) dist(matrixData, method=distanceMatric),
    ...
    pathway.heatmap=pheatmap(matrixData,fontsize_row=fontSize,fontsize_col=fontSize,color=my_palette, cluster_rows=F, cluster_cols=F, clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean", clustering_method = "ward.D2") 
    # pathway.heatmap=heatmap.2(matrixData, distfun=function(matrixData) dist(matrixData, method=distanceMatric),
    #                    col=my_palette,       # use on color palette defined earlier
    #                    symm=F,symkey=F,symbreaks=F, 
    #                    density.info="none",  # turns off density plot inside color legend
