...
cal.uniConSig<-function(target.list,feature.list,preCalmatrix,minsize=20,weight.cut=0.05,power=1,root=0.5,ECNpenalty=1,method="Ochiai",rm.overfit=TRUE){ #method=="Ochiai" or "Jaccard"
  ...
  target.weight=target.weight[as.numeric(target.weight$Target.Size)>minsize&as.numeric(target.weight$Weight)>weight.cut,]
...
run.weightedKS<-function(weight,feature.list,minsize=10,correct.overfit=FALSE,correct.outlier=FALSE){
...
weightedKSV2<-function(weights,compare.list,myPermu,p.cut=0.05,correct.overfit=FALSE){ 
  ...
  for(k in 1:length(compare.list)){
    ...
    if(numOnList<5|ES==0){ #required at least 5 index cell lines for each genotype or set NES to zero
      next;
    }else{
...
pathwayAssociation<-function(topPathway,compare.list,feature.list,preCalmatrix,topn=3,minsize=10){ #upPathways TRUE or FALSE
  ...
  for (i in 1:length(topPathway.list)){
    pathwayx=topPathway.list[[i]]
    tmp.uniConSig=cal.uniConSig(target.list=pathwayx,feature.list=feature.list,preCalmatrix=preCalmatrix,minsize=10,weight.cut=0.05,power=1,root=1,ECNpenalty=0.5,method="Ochiai")
    tmp.CSEA=CSEA2(target.score=setNames(as.numeric(tmp.uniConSig$uniConSig), tmp.uniConSig$subjectID),compare.list=topPathway.list,p.cut=1)
...
limmaDGE<-function(gctFile,clsFile){ # the comparison is based on increasingly sorted orders of group labels with positive t value showing upreulated in class 1
  ...
  limmaOut<-limmaOut[order(limmaOut$T.Value,decreasing = TRUE),]
  return(limmaOut)
}