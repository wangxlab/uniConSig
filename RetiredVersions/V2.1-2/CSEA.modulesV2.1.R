...
weightedKS.batch<-function(weight,feature.list,preCal=TRUE,high.enrich=TRUE,p.cut=0.05,correct.overfit=FALSE,correct.outlier=FALSE){
  if(high.enrich){
  ...
  ks.result<-weightedKSV2(tmp.weight,feature.list,myPermu,p.cut=p.cut,correct.overfit=correct.overfit)
  return(ks.result)
}
#run weighted KS for all cell lines of a drug ID using weightedKSV2 module
run.weightedKS<-function(weight,feature.list,minsize=5,correct.overfit=FALSE,correct.outlier=FALSE){
...
CSEA2<-function(target.score,compare.list,p.cut=0.05,minsize=5){
    ...
    print("Pre-calculation finished")
    ks.result<-weightedKSV2(tmp.weight,compare.list,myPermu,p.cut=p.cut)
...
benchmark.uniConSig<-function(tmp.weight,training.list,testing.list=NA){
  ...
  ks.result<-weightedKSV2(tmp.weight,compare.list,myPermu,p.cut=1)  
  return(ks.result)
}
...
limmaDGE<-function(gctFile,clsFile){ # the comparison is based on increasingly sorted orders of group labels with positive t value showing upreulated in class 1
  ...
  ###########################################
  log2Foldchange<-apply(myTableNumericDF,1,function(x){
  ...
  fit<-lmFit(myTableNumericDF,design=model.matrix(~fac))
