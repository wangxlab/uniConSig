...
weightedKS.batch<-function(weight,feature.list,preCal=TRUE,high.enrich=TRUE,p.cut=0.9,correct.overfit=FALSE,correct.outlier=FALSE,minsize=5){
  ...
  if(high.enrich){
    ...
    tmp.weight=sort(max(weight)-weight)   # this make the first element in the weight zero.  If minsize=0 or 1, line244 matchn is c(1,2,3,4,5).  Line 248 perm_weightedKS may find gene of weight=0. 
					 # cal_ES line 367, "sum(weight[onIndice]" will be zero and "weight[onIndice]/sum(weight[onIndice])" gets NA
  }
  if (correct.outlier){
  ...
  matchn=sort(unique(matchn))  # 1, 2, 3, 4, 5
  k=0
  for(j in matchn){     
      print(paste0("j: ", j))
...
run.weightedKS<-function(weight,signed=TRUE,feature.list,minsize=5,correct.overfit=FALSE,correct.outlier=FALSE,transformNegWeight=FALSE){
...
perm_weightedKS<-function(weights,numOnList,nPermu=1000){
  ...
  while(length(permu)<nPermu){
      print(paste0("perm_weightedKS permu: ", permu))
    tmp.genolist<-sample(names(weights),numOnList)
        print(paste0("tmp genolist: ", tmp.genolist))

          if(weights[tmp.genolist]!=0) {
	permES<-cal_ES(weights,tmp.genolist)
      	print(paste0("permES: ", permES))

	permu<-append(permu,permES)
	}
           if(length(permu)==5) break
  }
