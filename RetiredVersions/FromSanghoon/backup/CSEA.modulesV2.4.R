...
run.weightedKS<-function(weight,signed=TRUE,feature.list,minsize=10,correct.overfit=FALSE,correct.outlier=FALSE){
  ...
  if (signed==TRUE){
    ks.descending<-weightedKS.batch(weight=weight.transform(weight),feature.select,preCal = TRUE,high.enrich = TRUE,correct.overfit=correct.overfit,correct.outlier=correct.outlier)
    ks.ascending<-weightedKS.batch(weight=weight.transform(-weight),feature.select,preCal = TRUE,high.enrich = TRUE,correct.overfit=correct.overfit,correct.outlier=correct.outlier) 
  }else{
...
CSEA2<-function(target.score,compare.list,p.cut=0.05,minsize=5){
    ...
    print("Pre-calculation finished")
    ks.result<-weightedKSV2(tmp.weight,compare.list,myPermu=myPermu,p.cut=p.cut)
...
disambiguation<-function(CSEA.result,uniConSig.result,compare.list,upPathways,topn=30,p.cut=0.01){ #upPathways TRUE or FALSE
  topPathway=CSEA.result$Compare.List[1:topn]
  ...
  if (upPathways){
	...
	#write.table(CSEA.del, "CSEA.2.3.disambiguation.out_topPathhway.txt", sep="\t",quote=F, col.name=T, row.names=F)
  ...
  index=which(tmp.bin>0.01,arr.ind = TRUE)		# dim(tmp.bin) 30 30 length(tmp.bin) 900  dim(index) 386 2  
  remove=c()
  ...
  filter.result=CSEA.result[1:topn,]
...
pathwayAssociation<-function(topPathway,compare.list,feature.list,preCalmatrix,topn=3,minsize=10){ #upPathways TRUE or FALSE
  topPathway=topPathway[which(sapply(topPathway,function(x) length(compare.list[[x]])>=minsize))]
  topPathway.list=compare.list[topPathway]
  topPathway.CSEA=list()
  for (i in 1:length(topPathway.list)){
    pathwayx=topPathway.list[[i]]
    tmp.uniConSig=cal.uniConSig(target.list=pathwayx,feature.list=feature.list,preCalmatrix=preCalmatrix,minsize=minsize,weight.cut=0.05,power=1,root=1,ECNpenalty=0.5,method="Ochiai")
...
pathway.heatmap<-function(matrixData,clustering=TRUE,distanceMatric="euclidean"){
  ...
  if (clustering==TRUE){
    heatmap.2(matrixData, distfun=function(matrixData) dist(matrixData, method=distanceMatric),
                       ...
                       cexRow=0.7, cexCol=0.7,keysize=1,
                       offsetRow = 0,
                       offsetCol = 0,
                       margins = c(20, 25),
                       sepwidth=c(0.1,0.1),
...
## clean.counts function is necessary for scdeDEG
...
scdeDEG<-function(readCountGCT=readCountGCTFile,clsFile=clsFile, min.lib.size=100, min.reads=1,min.detected=1,SignPzscore=0.75)  {
	...
	# clean up the dataset (cd)
	#cleanData <- clean.counts(countUnique.df, min.lib.size=min.lib.size, min.reads=min.reads, min.detected=min.detected)  # min.lib.size=100, min.reads=1, min.detected=0  4094 177
	...
	expDiff <- scde.expression.difference(o.ifm, cleanData, o.prior, groups=groups, n.randomizations=100, n.cores=1, verbose=1)  #3700 6
	...
	expDiff.order.GeneSymb<-rownames_to_column(expDiff.order,"GeneSymb")
	expDiff.filtered<-expDiff.order.GeneSymb %>% dplyr::filter(abs(Signed.P.Zscore)>SignPzscore)    # filter out low abs(Signed.P.Zscore)   
	rownames(expDiff.filtered)<-expDiff.filtered$GeneSymb
	expDiff.filtered$GeneSymb<-NULL
	return(expDiff.filtered)
}
